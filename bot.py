# ========== bot.py (Render Webhook - Fully Async) ==========
import os
import sys
import time
import uuid
...
def set_webhook():
    ...
    try:
        ...
        result = loop.run_until_complete(bot_app.bot.set_webhook(webhook_url))
        loop.close()
...
@flask_app.route('/deletewebhook', methods=['GET'])
def delete_webhook():
    """Delete webhook"""
    global bot_app
    if bot_app is None:
    ...
    try:
        ...
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(bot_app.bot.delete_webhook())
        loop.close()
        return jsonify({'status': 'ok', 'result': result}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500

# ========== Botni ishga tushirish (Webhook rejimi) ==========
def initialize_bot():
    """Botni webhook rejimida ishga tushirish"""
    global bot_app
    try:
        reset_daily_limits()
        bot_app = Application.builder().token(TOKEN).connect_timeout(30).read_timeout(30).build()
        
        bot_app.add_handler(CommandHandler("start", start))
        bot_app.add_handler(CommandHandler("referral", referral_command))
        bot_app.add_handler(CommandHandler("add_points", add_points_command))
        bot_app.add_handler(CommandHandler("add_vip", add_vip_command))
        ...
        bot_app.add_handler(MessageHandler(filters.PHOTO, handle_messages))
        bot_app.add_handler(MessageHandler(filters.VIDEO, handle_messages))
        
        # Bot ma'lumotlarini olish
        import asyncio
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        me = loop.run_until_complete(bot_app.bot.get_me())
        loop.close()
        logger.info(f"🚀 Bot {me.username} ishga tushdi!")
        
        # Webhook o'rnatish (Render URL mavjud bo'lsa)
        render_url = os.environ.get('RENDER_EXTERNAL_URL', '')
        if render_url:
            webhook_url = f"{render_url}/webhook"
            loop = asyncio.new_event_loop()
            ...
            logger.warning("⚠️ RENDER_EXTERNAL_URL topilmadi, webhook o'rnatilmadi")
